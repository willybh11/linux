...
#define OF_ADRV9002_DPD(key, def, min, max, val)	\
	 ...
	 * Giving the relation between the signal/peak would also not be optimal since the
	 * maximum resolution we could go (without overflowing ULL) would be nano which could
	 * lead to some approximations. I'm no really 100% sure these approximations would matter,
	 * so better not to take any chances and just let the user give raw values. Some help
	 * can be given on how to get here though... Assuming we know the dBFS value:
	...
	tx->dpd->resetLuts = of_property_read_bool(dpd, "adi,reset-lut");
